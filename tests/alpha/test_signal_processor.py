...
class TestDynamicSignalProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.stock_returns = pd.DataFrame(np.random.randn(300, 2) * 0.01, index=dates, columns=self.tickers)
    ...
    def test_signal_decay(self):
        ...
        self.mock_signals_data['MSFT']['insider_trades']['date'] = [self.current_date - pd.Timedelta(days=30)] # 1 half-life old
        ...
        self.assertAlmostEqual(decayed_scores.loc['AAPL', 'alpha_score'], expected_aapl_score, places=5)
        self.assertAlmostEqual(decayed_scores.loc['MSFT', 'alpha_score'], expected_msft_score, places=5)
    ...
    def test_cross_asset_alpha_logic(self):
        ...
        alpha_scores_pos = self.processor.generate_cross_asset_alpha(stock_returns_negative, spy_returns_positive)
        self.assertTrue(all(alpha_scores_pos > 0), "Expected positive alpha when bonds outperform stocks")
        ...
        alpha_scores_neg = self.processor.generate_cross_asset_alpha(stock_returns_positive, spy_returns_negative)
        self.assertTrue(all(alpha_scores_neg < 0), "Expected negative alpha when stocks outperform bonds")
    ...
    def test_cross_asset_alpha_insufficient_data(self):
        ...
        alpha_scores = self.processor.generate_cross_asset_alpha(short_stock_returns, short_spy_returns)
