...
def build_hybrid_feature_set(current_date, returns_data, raw_fmp_signals, trends_data, lookback_days, forward_days):
    ...
    # Simulate features
    X = pd.DataFrame(np.random.rand(num_days * num_assets, 4),
...
def get_hybrid_prediction_features(current_date, returns_data, raw_fmp_signals, trends_data):
    ...
    num_assets = len(returns_data.columns)
    X_pred = pd.DataFrame(np.random.rand(num_assets, 4),
                          columns=['mom_1m', 'mom_12m', 'fmp_alpha', 'trends_sentiment'],
...
def main():
    ...
    trends_data = trends_loader.get_trends_for_universe(universe, start_date='2010-01-01', end_date=BACKTEST_END_DATE)
    ...
    rebalance_dates = pd.date_range(start=BACKTEST_START_DATE, end=BACKTEST_END_DATE, freq='Q').to_series()
    ...
    if weights_df.empty:
        logging.error("Backtest generated no weights. Performance file will not be created. Exiting.")
        return
    ...
    if daily_returns.empty:
        logging.error("Backtest generated no returns. Performance file will not be created. Exiting.")
        return
