...
class BaseSignalProcessor:
    ...
    def __init__(self, signals_data: Dict[str, Dict[str, pd.DataFrame]], selected_features: list = None):
...
class SignalProcessor(BaseSignalProcessor):
    ...
    def generate_composite_alpha_scores(self) -> pd.DataFrame:
        ...
        for ticker, data in self.signals_data.items():
            scores = [
                self.feature_map[name](data) for name in features_to_use 
...
class DynamicSignalProcessor(SignalProcessor):
    ...
    """
    def __init__(self, signals_data: Dict[str, Dict[str, pd.DataFrame]], decay_halflife_days: int = 30, selected_features: list = None):
        super().__init__(signals_data, selected_features)
    ...
    def _score_analyst_recs_decay(self, ticker_data: Dict[str, pd.DataFrame], current_date: pd.Timestamp) -> float:
        ...
        recs = recs.loc[(recs['date'] <= current_date) & (recs['date'] >= current_date - pd.Timedelta(days=252))]
        if recs.empty:
            return 0.0
    ...
    def _score_insider_trades_decay(self, ticker_data: Dict[str, pd.DataFrame], current_date: pd.Timestamp) -> float:
        ...
        # Filter for the last year of signals to avoid noise from very old data
        trades = trades.loc[(trades['date'] <= current_date) & (trades['date'] >= current_date - pd.Timedelta(days=252))]
        if trades.empty:
            return 0.0
        ...
        buys = (trades[trades['transactionType'] == 'P-Purchase']['transactionValue'] * trades['weight']).sum()
        sells = (trades[trades['transactionType'] == 'S-Sale']['transactionValue'] * trades['weight']).sum()
    ...
    def generate_time_aware_alpha_scores(self, current_date: pd.Timestamp) -> pd.DataFrame:
        ...
        for ticker, data in self.signals_data.items():
            ...
            for name in features_to_use:
                if name in self.feature_map and data.get(name) is not None and not data.get(name).empty:
...
class CrossAssetAlphaProcessor(DynamicSignalProcessor):
    ...
    def generate_combined_alpha(
        ...
        fmp_alpha, cross_asset_scores = fmp_alpha.align(cross_asset_scores, join='right', fill_value=0)
        ...
        logger.info(f"Generated combined alpha for {current_date}: mean={combined_alpha.mean():.3f}, std={combined_alpha.std():.3f}")
    ...
    def _calculate_cross_asset_momentum(
        ...
        if len(spy_returns) < lookback or len(stock_returns) < lookback:
            logging.warning(f"Not enough data for cross-asset alpha (need {lookback} days). Returning zero scores.")
