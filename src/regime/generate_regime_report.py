...
def generate_regime_plot(prices: pd.DataFrame, results_dir: str):
    ...
    ax.plot(
        market_prices.index, market_prices, label="Market Proxy (SPY)", color="black", alpha=0.7
    )
    ax.plot(
        sma_short.index,
        sma_short,
        label=f"SMA-{detector.lookback_short}",
        color="blue",
        linestyle="--",
    )
    ax.plot(
        sma_long.index, sma_long, label=f"SMA-{detector.lookback_long}", color="red", linestyle="--"
    )
...
def main():
    ...
    if not os.path.exists(data_path):
        logging.error(
            f"Data file not found at {data_path}. Please run a backtest first to generate it."
        )
        return
